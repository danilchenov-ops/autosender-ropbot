"""Ночной снимок «Visual»: баллы дня по обобщённой модели менеджера.

Модель (веса согласованы с вкладкой KPI, решения Тимофея 28.08.2026):
- Новые заявки дня (шаг 0): за каждую живую заявку — 0–10 баллов
  (60% настойчивость-звонки, 40% скорость первого звонка; скорость
  судится только по будним заявкам 10–18 Влд).
- Смены статусов дня (шаги 1–3): за каждое окно — 0–100 баллов
  (70% скорость первого звонка после статуса, 30% охват). Вес x10
  к заявке — перекалибровка 29.08.2026 по реальным продажам март–август:
  «толкание» середины воронки ранжирует менеджеров по продажам лучше
  всего (Spearman 0.47 против 0.20 у старых весов). Окна короче
  5 минут (задним числом) баллов не приносят.
- Сопровождение торгов (шаг 4): за каждый сегодняшний звонок клиенту
  на открытой стадии «На торгах» — 10 баллов, максимум 30 в день на заявку.
- Продажи в балл НЕ входят — убраны решением Тимофея 29.08.2026
  (раньше было 100 баллов за завершённую сделку deal → WON).

Вкладка Visual показывает баллы ПО НЕДЕЛЯМ (решение Тимофея 29.08) с эталоном:
parts->'ideal' — балл дня при работе строго по правилам с тем же потоком
(10 за заявку, 10 за окно статуса, 2 за ежедневное касание каждого
открытого торга). Старт 15.07.2026, прошлое досчитано visual_backfill.py.
Запуск кроном раз в день в 23:55 Влд (16:55 MSK), перезапуск за тот же
день безопасен (upsert перезаписывает день целиком).
"""
...
def main():
    ...
    # ── шаг 0: заявки, созданные сегодня ─────────────────────────────────────
    rows = conn.execute("""
    ...
    # ── шаги 1–3: окна статусов, открытые сегодня ────────────────────────────
    rows = conn.execute("""
          ...
          LEFT JOIN LATERAL (
              SELECT s.created_time AS t_end FROM stage_history s
               WHERE s.entity_kind='lead' AND s.owner_id=ev.owner_id
                 AND s.created_time>ev.t0 AND s.stage_id<>ev.stage_id
    ...
    # ── шаг 4: сегодняшние звонки по открытым торгам ─────────────────────────
    rows = conn.execute("""
        SELECT l.assigned_by, count(*) AS calls_today
          ...
          JOIN leads l ON l.id = ev.owner_id
          JOIN calls c
            ON c.direction='out' AND c.call_start >= %s
           AND ((c.crm_entity_type='LEAD' AND c.crm_entity_id=l.id)
         ...
         GROUP BY l.assigned_by, l.id""", (d0,)).fetchall()
    for uid, calls_today in rows:
        if uid not in pts:
            continue
        pts[uid]["s4"] += 10 * min(calls_today, 3)

    # эталон торгов: ежедневное касание каждого открытого торга (по 2 балла)
    rows = conn.execute("""
         ...
         WHERE NOT EXISTS (
               SELECT 1 FROM stage_history s
                WHERE s.entity_kind='lead' AND s.owner_id=ev.owner_id
                  AND s.created_time>ev.t0
                  AND (s.stage_id='13' OR s.stage_semantic IN ('F','S')))
         GROUP BY l.assigned_by""").fetchall()
    ...
    for uid, p in pts.items():
        ...
        conn.execute("""
            ...
            DO UPDATE SET pts = EXCLUDED.pts, parts = EXCLUDED.parts,
                          computed_at = now()""",
            (day, uid, total, json.dumps({k: round(v, 1)
                                          for k, v in p.items()})))
    print(f"{day}: " + ", ".join(
